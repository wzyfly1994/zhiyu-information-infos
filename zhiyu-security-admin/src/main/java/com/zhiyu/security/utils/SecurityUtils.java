 ...
 */
package com.zhiyu.security.utils;

import com.alibaba.fastjson.JSON;
import com.alibaba.fastjson.JSONArray;
import com.alibaba.fastjson.JSONObject;
import com.zhiyu.core.exception.BusinessException;
import com.zhiyu.security.enums.DataScopeEnum;
...
public class SecurityUtils {
    ...
    public static String getCurrentUsername() {
        ...
        if (authentication == null) {
            throw new BusinessException("当前登录状态过期", HttpStatus.UNAUTHORIZED.value());
        }
        if (authentication.getPrincipal() instanceof UserDetails) {
            UserDetails userDetails = (UserDetails) authentication.getPrincipal();
            return userDetails.getUsername();
        }
        throw new BusinessException("找不到当前登录的信息", HttpStatus.UNAUTHORIZED.value());
    }
